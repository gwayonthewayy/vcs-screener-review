...
`ANALYSIS_READY_LIQUIDITY_RATIO=0.5` is a heuristic for the analyzable-liquidity denominator only. It is not a coverage-threshold bypass and it does not relax the 70% publish gate. If the ratio changes, update the coverage tests and live refresh evidence together.
...
The frozen positive fixture verifies three practical symbols in identical Markdown, Telegram, and TradingView order, disjoint observation output, and no observation backfill when practical count is zero. A builder-level fixture also exercises `scripts/build_market_briefing.py` against real temporary files to confirm the same selector order reaches Markdown, Telegram Top, and TradingView main outputs. The current regression set reaches the real file I/O path and the full suite contains 64 tests.

The latest live isolated refresh reached KR `79.2%` and US `91.9%` coverage, generated a fresh weekly snapshot, and the daily dry-run consumed that snapshot without backfilling observation names into practical Top sections. Live positive path: PASS. The full suite now contains 64 tests.
